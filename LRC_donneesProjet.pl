...
subsS(C, D, L) :- subs(C, D), not(member(D, L)).
subsS(C, D, L) :- subs(D, C), not(member(C, L)).
subsS(all(R, C), all(R, D), L) :- subsS(C, D, L).
subsS(C, D, L) :- equiv(C, E), subsS(E, D, L). %règles pour l'équivalence, nécessaires pour l'exo4%
subsS(C, D, L) :- equiv(D, E), subsS(C, E, L).
